...
## eg
- ### eg：$`\int{\cos^5{x}\,dx}`$
    - ### Pythagorean Identities：$`\int{\cos^5{x}\,dx} = \int{\cos^4{x}\cos{x}\,dx} = \int{\left(1-\sin^2{x}\right)^2\cos{x}\,dx}`$
...
- ### eg：$`\int{\cos^4{x}\,dx}`$
    - ### Power Reduction Formulas：$`\int{\cos^4{x}\,dx} = \int{\left(\frac{1+\cos{2x}}{2}\right)^2\,dx}`$
