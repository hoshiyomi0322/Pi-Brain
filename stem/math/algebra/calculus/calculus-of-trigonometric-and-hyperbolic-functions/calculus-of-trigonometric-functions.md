# Derivatives of [Trigonometric Functions](/stem/math/trigonometric-and-hyperbolic-functions/trigonometric-functions.md)
|Derivatives with Positive Sign|Derivatives with Negative Sign ([Cofunction](../../../trigonometric-and-hyperbolic-functions/trigonometric-functions.md#complementary-function-cofunction))|
|:---:|:---:|
|$`\left(\sin{x}\right)^\prime=\cos{x}`$|$`\left(\cos{x}\right)^\prime=-\sin{x}`$|
|$`\left(\tan{x}\right)^\prime=\frac{1}{\cos^2{x}}=\sec^2{x}`$|$`\left(\cot{x}\right)^\prime=-\frac{1}{\sin^2{x}}=-\csc^2{x}`$|
|$`\left(\sec{x}\right)^\prime=\frac{\sin{x}}{\cos^2{x}}=\sec{x}\tan{x}`$|$`\left(\csc{x}\right)^\prime=-\frac{\cos{x}}{\sin^2{x}}=-\csc{x}\cot{x}`$|
# Integrals of [Trigonometric Functions](/stem/math/trigonometric-and-hyperbolic-functions/trigonometric-functions.md)
- ### $`\int{\sin{x}\, dx}=-\cos{x}+C`$
- ### $`\int{\cos{x}\, dx}=\sin{x}+C`$
- ### $`\int{\tan{x}\, dx}=-\ln{\left|\cos{x}\right|}+C=\ln{\left|\sec{x}\right|}+C`$
- ### $`\int{\cot{x}\, dx}=\ln{\left|\sin{x}\right|}+C=-\ln{\left|\csc{x}\right|}+C`$
- ### $`\int{\sec{x}\, dx}=\ln{\left|\sec{x}+\tan{x}\right|}+C`$
- ### $`\int{\csc{x}\, dx}=-\ln{\left|\csc{x}+\cot{x}\right|}+C`$

# $`\int{\sec^2{x}\, dx},~\int{\csc^2{x}\, dx}`$
- ### $`\int{\sec^2{x}\, dx}=\tan{x}+C`$
- ### $`\int{\csc^2{x}\, dx}=-\cot{x}+C`$
- ### [Pythagorean Identities](/stem/math/trigonometric-and-hyperbolic-functions/trigonometric-functions.md#pythagorean-identities)
    - ### $`\int{\tan^2{x}\, dx}=\int{\left(\sec^2{x}-1\right)\, dx}=\tan{x}-x+C`$
    - ### $`\int{\cot^2{x}\, dx}=\int{\left(\csc^2{x}-1\right)\, dx}=-\cot{x}-x+C`$

# Reduction Formula
- ### [Reduction Formula](reduction-formula.md)
